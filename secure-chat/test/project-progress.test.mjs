...
test("project progress is derived only from explicit test and operation gates", () => {
  ...
  assert.deepEqual(progress.tests, { done: 28, total: 32, percent: 88 });
  assert.deepEqual(progress.operations, { done: 7, total: 32, percent: 22 });
  assert.deepEqual(progress.overall, { done: 35, total: 64, percent: 55 });
  assert.equal(progress.stages[0].combinedPercent, 50);
});

test("progress report separates tested components from live user capability", () => {
  const report = formatProjectProgress();
  assert.match(report, /전체 검증 게이트: 55% \(35\/64\)/u);
  assert.match(report, /자동시험·격리 구현: 88% \(28\/32\)/u);
  assert.match(report, /실제 운영 연결: 22% \(7\/32\)/u);
