...
test("project progress is derived only from explicit test and operation gates", () => {
  ...
  assert.deepEqual(progress.operations, { done: 10, total: 32, percent: 31 });
  assert.deepEqual(progress.overall, { done: 39, total: 64, percent: 61 });
...
test("progress report separates tested components from live user capability", () => {
  ...
  assert.match(report, /전체 검증 게이트: 61% \(39\/64\)/u);
  assert.match(report, /자동시험·격리 구현: 91% \(29\/32\)/u);
  assert.match(report, /실제 운영 연결: 31% \(10\/32\)/u);
